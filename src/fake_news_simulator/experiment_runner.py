...
class ExperimentRunner:
    ...
    @staticmethod
    def run(scenarios: list[Scenario]) -> list[SimulationResult]:
        ...
        total_simulations = len(scenarios) * runs_per_scenario
        completed_simulations = 0

        for run_index in range(1, runs_per_scenario + 1):
            print(f"Creating graph for run {run_index}/{runs_per_scenario}")
